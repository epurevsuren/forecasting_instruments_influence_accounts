...
Outputs: truth_training_set_FINAL.csv        (no hard tiers, continuous honest labels)
         ...
         trump_truths_labeled.csv            (full metadata; also the incremental dedup ledger)

CLI:
  python build_final_training_set.py          -> INCREMENTAL (daily): label only posts in
                                                 trump_truths_scored.csv not yet in
                                                 trump_truths_labeled.csv, APPEND to all 3 outputs
...
def train_columns(impact_cols):
    return ['date','text','sample_weight','nlp_signal'] + impact_cols
...
def main_incremental():
    ...
    if set(final_cols) != set(train_cols):
        print(f"  ⚠️  Column mismatch vs existing {os.path.basename(FINAL_CSV)} — "
